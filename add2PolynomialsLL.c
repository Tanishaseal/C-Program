// C program to add two polynomials of linked list
#include <stdio.h>
#include <stdlib.h>
struct Node {
    int coeff;
...
int main() {
    ...
    struct Node* head1 = createNode(5, 2);//5x^2
    head1->next = createNode(4, 1);//4x^1
    head1->next->next = createNode(2, 0);//2x^0

    // 2nd polynomial: -5x^1-5x^0
    struct Node* head2 = createNode(-5, 1);//-5x^1
    head2->next = createNode(-5, 0);//-5x^0
